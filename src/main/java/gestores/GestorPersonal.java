...
public class GestorPersonal {
    ...
    public boolean actualizarEmpleado(Empleado empleado) throws BDException{
         String query = "UPDATE EMPLEADO SET Nombre = ?, Jornada_LABORAL = ?, Salario = ?, Fecha_contratacion = ?, Estado = ?, Rol = ? WHERE DPI = ?"; 
         try (Connection connection = conexionDB.getConnection();
             PreparedStatement ps = connection.prepareStatement(query)){
             
            ps.setString(1, empleado.getNombre());
            ps.setString(2, empleado.getJornadaLaboral());
            ...
            ps.setString(7, empleado.getDpi());
            int filasAfectadas = ps.executeUpdate();
            return filasAfectadas > 0;
    ...
    public List<Empleado> listarEmpleados() throws BDException{
